 ...
 */
class GetContainerAcl extends JAzureQuery
{
	/**
	 ...
	 * @var string
	 */
	protected $name = 'getContainerAcl';
	...
	protected function processResults($results) {
		if ($results instanceof GetContainerAclResult) {
			$acl = $results->getContainerAcl()->getPublicAccess();
			return $acl;
			if (is_null($acl)) {
				return PublicAccessType::NONE;
			} elseif ($acl == 'blob') {
				return PublicAccessType::BLOBS_ONLY;
			} elseif ($acl == 'container') {
				return PublicAccessType::CONTAINER_AND_BLOBS;
			}
		}
		return false;
	}
